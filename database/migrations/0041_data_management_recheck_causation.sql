...
'Purpose-bound reassessment-request command. It requires an active tenant-local assessment with every projected gap backed by accepted evidence, binds the evidence-accepted outbox event whose recorded next action proves that acceptance causally closed the final gap, appends immutable request evidence, and emits one causally linked privacy-minimized transactional outbox event. Exact UUIDv7 decision replay is idempotent; conflicting meaning fails closed. PUBLIC execution is revoked until an authenticated application boundary is implemented.';
