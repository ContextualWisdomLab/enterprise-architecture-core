...
CREATE FUNCTION architecture_core.start_scheduled_transformation(
...
DECLARE
  ...
  schedule_effective_at timestamptz;
...
BEGIN
  ...
  IF uuid_extract_version(requested_transformation_id) <> 7
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state start identifiers must be UUIDv7';
  END IF;
  ...
  IF length(btrim(requested_decision_actor_ref)) NOT BETWEEN 1 AND 2048
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'start actor and reason must be non-empty and bounded';
  END IF;
  ...
  IF previous_sequence_number IS NULL OR previous_state_code <> 'approved' THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state start requires the current transformation state to be approved';
  END IF;

  IF requested_effective_at < previous_effective_at THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state start cannot move backward from approval time';
  END IF;

  SELECT schedule_record.effective_at
    INTO schedule_effective_at
   ...
   WHERE schedule_record.tenant_record_id = requested_tenant_record_id
     AND schedule_record.architecture_transformation_id = requested_transformation_id
     AND schedule_record.superseded_at IS NULL
     AND schedule_record.truth_status_code = 'authoritative'
   FOR SHARE;

  IF schedule_effective_at IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state start requires an active authoritative schedule binding';
  END IF;

  IF requested_effective_at < schedule_effective_at THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state start cannot precede the governed schedule';
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE
      'GRANT EXECUTE ON FUNCTION architecture_core.start_scheduled_transformation('
      'uuid,uuid,uuid,timestamptz,text,text,uuid) TO ea_runtime';
  END IF;
END;
$$;

COMMENT ON FUNCTION architecture_core.start_scheduled_transformation(
    uuid,
    uuid,
    uuid,
    timestamptz,
