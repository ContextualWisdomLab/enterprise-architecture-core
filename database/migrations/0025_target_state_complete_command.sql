...
CREATE FUNCTION architecture_core.complete_started_transformation(
...
BEGIN
  ...
  IF uuid_extract_version(requested_transformation_id) <> 7
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state completion identifiers must be UUIDv7';
  END IF;
  ...
  IF length(btrim(requested_decision_actor_ref)) NOT BETWEEN 1 AND 2048
    ...
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'completion actor and reason must be non-empty and bounded';
  END IF;
  ...
  -- Exact replay may return immediately. Fresh requests serialize on the
  -- authoritative transformation, then recheck the decision id so concurrent
  -- delivery converges on one immutable history/outbox receipt.
  LOOP
    SELECT
    ...
    IF existing_history_id IS NOT NULL THEN
      IF existing_transformation_id IS DISTINCT FROM requested_transformation_id
         OR existing_state_code IS DISTINCT FROM 'completed'
        ...
        RAISE EXCEPTION USING
          ERRCODE = '23505',
          MESSAGE = 'decision request id already represents different completion meaning';
      END IF;
      ...
      IF existing_event_id IS NULL THEN
        RAISE EXCEPTION USING
          ERRCODE = '23514',
          MESSAGE = 'completed history exists without transactional outbox evidence';
      END IF;
  ...
  IF previous_sequence_number IS NULL OR previous_state_code <> 'started' THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state completion requires the current transformation state to be started';
  END IF;

  IF requested_effective_at < previous_effective_at THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state completion cannot move backward from start time';
  END IF;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE
      'GRANT EXECUTE ON FUNCTION architecture_core.complete_started_transformation('
      'uuid,uuid,uuid,timestamptz,text,text,uuid) TO ea_runtime';
  END IF;
END;
$$;

COMMENT ON FUNCTION architecture_core.complete_started_transformation(
    uuid,
    uuid,
    uuid,
    timestamptz,
...
'Purpose-bound human completion command that advances one started authoritative EA transformation to completed. The caller must already have verified Keyverse signature, issuer, audience, expiration, tenant and completion role. One UUIDv7 decision request is idempotent: exact replay returns the original history/outbox receipt, conflicting meaning is rejected, concurrent same-key delivery serializes on the authoritative transformation before replay recheck, completion cannot predate start, and authoritative append-only history plus a privacy-minimized outbox event commit atomically.';
