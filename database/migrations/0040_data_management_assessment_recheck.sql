...
CREATE TABLE architecture_core.assessment_recheck_request (
    tenant_record_id uuid NOT NULL,
    assessment_recheck_request_id uuid NOT NULL DEFAULT uuidv7(),
    data_management_assessment_projection_id uuid NOT NULL,
    trigger_evidence_acceptance_id uuid NOT NULL,
    decision_request_id uuid NOT NULL,
    requested_at timestamptz NOT NULL,
    recorded_at timestamptz NOT NULL DEFAULT clock_timestamp(),
    CONSTRAINT assessment_recheck_request_primary_key
        PRIMARY KEY (tenant_record_id, assessment_recheck_request_id),
    CONSTRAINT assessment_recheck_request_projection_foreign
        FOREIGN KEY (
            tenant_record_id,
    ...
    CONSTRAINT assessment_recheck_request_acceptance_foreign
        FOREIGN KEY (tenant_record_id, trigger_evidence_acceptance_id)
        REFERENCES architecture_core.assessment_evidence_acceptance
            (tenant_record_id, assessment_evidence_acceptance_id),
    CONSTRAINT assessment_recheck_request_uuid_version
        CHECK (uuid_extract_version(assessment_recheck_request_id) = 7),
    CONSTRAINT assessment_recheck_request_decision_uuid_version
        CHECK (uuid_extract_version(decision_request_id) = 7),
    CONSTRAINT assessment_recheck_request_projection_unique
        UNIQUE (tenant_record_id, data_management_assessment_projection_id),
    CONSTRAINT assessment_recheck_request_decision_unique
...
CREATE FUNCTION architecture_core.reject_assessment_recheck_request_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  RAISE EXCEPTION USING
    ERRCODE = '23514',
    MESSAGE = 'assessment reassessment-request evidence is immutable';
END;
$$;

CREATE TRIGGER assessment_recheck_request_history_guard
BEFORE UPDATE OR DELETE
ON architecture_core.assessment_recheck_request
FOR EACH ROW
EXECUTE FUNCTION architecture_core.reject_assessment_recheck_request_mutation();
...
COMMENT ON TABLE architecture_core.assessment_recheck_request IS
'Immutable tenant-scoped evidence that every projected data-management gap was closed and a specific final evidence acceptance triggered reassessment. The foreign Data/AI Context assessment remains a projection reference rather than copied authority.';
...
'Purpose-bound reassessment-request command. It requires an active tenant-local assessment with every projected gap backed by accepted evidence, binds the latest acceptance that closed the final gap, appends immutable request evidence, and emits one causally linked privacy-minimized transactional outbox event. Exact UUIDv7 decision replay is idempotent; conflicting meaning fails closed. PUBLIC execution is revoked until an authenticated application boundary is implemented.';
