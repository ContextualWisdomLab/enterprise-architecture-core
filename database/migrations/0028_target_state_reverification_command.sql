...
-- Monitoring may discover that previously verified target-state evidence has
-- become stale. Preserve the immutable execution/history model by allowing a
-- new human-authorized verification observation after an existing verified
-- observation. A detected gap remains terminal and requires a new governed
-- plan/transformation rather than reopening the old execution.
...
BEGIN
  ...
  transition_allowed := CASE previous_state_code
    ...
    WHEN 'verified' THEN NEW.transformation_state_code IN ('verified', 'gap_detected')
    ELSE false
  END;
...
CREATE OR REPLACE FUNCTION architecture_core.record_target_state_verification(
...
BEGIN
  ...
  IF previous_sequence_number IS NULL
     OR previous_state_code NOT IN ('completed', 'verified') THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state verification requires completed or previously verified transformation evidence';
  END IF;

  IF requested_effective_at < previous_effective_at THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'target-state verification cannot predate prior execution or verification evidence';
  END IF;
...
COMMENT ON FUNCTION architecture_core.record_target_state_verification(
    ...
    uuid,
    text
) IS 'Appends evidence-backed target-state verification or re-verification history and atomic outbox evidence after completed/verified execution state.';
