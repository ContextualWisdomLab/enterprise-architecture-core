...
- Deterministic buyer next-action states for recorded lifecycle risk and incomplete evidence (`monitor`, `plan_target_state`, `start_remediation`, `complete_capability_mapping`, `complete_lifecycle_evidence`), with bounded planning horizons; the projector exposes the next risk-bearing lifecycle transition and its evidence while ignoring mutable non-temporal support metadata for historical classification.
