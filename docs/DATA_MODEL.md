...
Projection insertion fails closed unless the receipt is already `processed`, its `processed_at` is no later than the local projection `recorded_at`, the referenced EA object is an application, and the source product obeys its responsibility boundary. Canonical foreign URIs must encode the same tenant, owner, and kind as the reference row. Migration 0018 also requires every `projection_receipt.event_source_uri` to satisfy the exact Context Graph canonical-authority URI grammar rather than merely exposing tenant/product text at expected colon positions. Composite tenant foreign keys and forced RLS protect both projection tables.
