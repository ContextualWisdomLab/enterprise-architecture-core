...
The bounded owner path is `src/ea_core_foundation/strategy_transformation/`. Transformation-completion command behavior lives in `strategy_transformation/complete.py`, and target-state monitoring freshness/read behavior lives in `strategy_transformation/monitor.py`. `completion_runtime.py` and `monitoring_runtime.py` compose those canonical owners directly. The historical root `complete.py` and `monitor.py` modules are behavior-free compatibility facades preserving existing public import objects. These path moves do not transfer authorization or persistence authority: completion still uses the EA-owned tenant-scoped stored command and transactional outbox semantics, while monitoring remains a purpose-authorized tenant-bound read through the EA-owned PostgreSQL query port with separate valid/business and recorded/system cutoffs.
