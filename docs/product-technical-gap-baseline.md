...
**Snapshot:** 2026-08-21T07:08:01Z, GitHub inspection, repository inspection, and local validation probe<br>
...
PR36 is now `d6e425182e4e5cca34336d3c8bcb4dcded6f9c71` after the current PR26
workflow-credential hardening was propagated through the stack; a fresh runtime
...
`d6e4251`. These are local candidate-head results, not protected integration
or release evidence.

**Live refresh:** `2026-08-21T07:08:01Z` — PR27→PR29→PR30→PR31→PR32→PR33→PR35→PR36
now form an exact synchronized stack at
`5144e9d10ebf59c2c31ce8a8e3d4fc0152726dfa` →
`178a483d328d961ab5ad4ad29271e7bc86a8559f` →
`a1da69682b7dc54b9410b2310a7fb8ebaceb34a7` →
`2a81aef48c9fc333bc871672ab9f0cc6cf848db5` →
`59a560e72863e9609e86f13e7b1d9617cc5fa934` →
`723b310281442b741713648dbe26446631b0a989` →
`70594b83253b6c077bb1355c17ece298c22746e5` →
`d6e425182e4e5cca34336d3c8bcb4dcded6f9c71`. PR26 is current at
`371dfe32153192a3e11fcd41016dd82f0a3d70bd`, and PR11 is current at
...
`371dfe32153192a3e11fcd41016dd82f0a3d70bd` with transaction-isolated planner
...
`723b310281442b741713648dbe26446631b0a989`, with the same clean PostgreSQL
...
`d6e425182e4e5cca34336d3c8bcb4dcded6f9c71` with the operator runbook and
...
Checks pending.
PR #29's propagated exact head `178a483d328d961ab5ad4ad29271e7bc86a8559f`
and PR #30's propagated exact head `a1da69682b7dc54b9410b2310a7fb8ebaceb34a7`
...
| Hot-write capacity preparation | Migration 0050 and ADR 0025 on PR #35 exact head `70594b83253b6c077bb1355c17ece298c22746e5` define deterministic tenant-derived 16-bucket routing, storage headroom, and hot-write indexes; PR #36 exact head `d6e425182e4e5cca34336d3c8bcb4dcded6f9c71` adds a read-only tenant-scoped capacity snapshot, failure-safe context restoration, and operator runbook for row volume, queue lag, bucket, relation/index size, and cumulative tuple/WAL comparison; the preceding stack passed 63 clean PostgreSQL 18.6 SQL acceptance files, while a fresh database/runtime probe for the current head remains pending; physical partition deployment is not claimed | A buyer gets a repeatable measurement baseline, while production capacity and partition cutover still require authorized representative-data evidence. |
...
| ---: | --- | --- | --- | --- |
| [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) | Add repeatable hot-write capacity snapshot | `hot-write-capacity` at current base `70594b83253b6c077bb1355c17ece298c22746e5` → `codex/hot-write-capacity-report` | Ready; exact head `d6e425182e4e5cca34336d3c8bcb4dcded6f9c71`; current stack passes 705 Python tests with 100% statement-and-branch coverage, Ruff, actionlint, repository validation, and `uv lock --check`; prior stack PostgreSQL evidence is historical and fresh current-head runtime/database evidence remains pending; hosted Checks are queued; no formal approval | Obtain an independent same-head review and terminal Checks; merge only after the exact current stack is valid. |
| [35](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/35) | Prepare append-only write boundaries for hot partitioning | `portfolio-assessment-summary` at current base `723b310281442b741713648dbe26446631b0a989` → `hot-write-capacity` | Ready; exact head `70594b83253b6c077bb1355c17ece298c22746e5`; current parent propagation retains the scoped inferred-dimension assertion and the stack’s 705-test/100%-coverage, Ruff, and repository evidence; hosted Checks are queued; no formal approval | Obtain terminal Checks and an independent same-head review; merge only after the exact current stack is valid. |
...
| [33](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/33) | Publish deterministic portfolio assessment summary | `portfolio-assessment-api` at current base `59a560e72863e9609e86f13e7b1d9617cc5fa934` → `portfolio-assessment-summary` | Ready; exact head `723b310281442b741713648dbe26446631b0a989`; portfolio contract count remains 14 operations, the inferred fixture assertion is dimension-scoped, and 705 local Python tests pass with 100% coverage plus current parent checks; hosted Checks are queued; no formal approval | Obtain terminal Checks and an independent same-head review. |
| [32](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/32) | Expose portfolio assessment read port | `data-management-recheck-status-v1` at base `2a81aef48c9fc333bc871672ab9f0cc6cf848db5` → `portfolio-assessment-api` | Ready; exact head `59a560e72863e9609e86f13e7b1d9617cc5fa934`; current parent propagation preserves the PR31 status fixes, dimension-scoped portfolio acceptance, 684 tests with 100% coverage, Ruff, repository validation, and lock validation; hosted Checks are queued; no formal approval | Obtain terminal Checks and an independent same-head review. |
| [31](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/31) | Expose data-management reassessment status | `data-management-recheck-v1` at current base `a1da69682b7dc54b9410b2310a7fb8ebaceb34a7` → `data-management-recheck-status-v1` | Ready; exact head `2a81aef48c9fc333bc871672ab9f0cc6cf848db5`; 651 tests with 100% statement-and-branch coverage, Ruff, repository validation, and lock validation pass after the fixture/validator repair; hosted Checks are queued; no formal approval | Obtain terminal Checks and an independent same-head review before advancing the stack. |
| [30](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/30) | Request data-management assessment recheck | `data-management-evidence-closure-v1` at current base `178a483d328d961ab5ad4ad29271e7bc86a8559f` → `data-management-recheck-v1` | Ready; exact head `a1da69682b7dc54b9410b2310a7fb8ebaceb34a7`; 616 tests with 100% coverage, repository validation, actionlint, and `uv lock --check` pass after parent propagation; prior clean PostgreSQL evidence remains historical; hosted Checks are queued; no formal approval | Obtain terminal hosted Checks and an independent same-head review before advancing the stack. |
| [29](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/29) | Accept data-management evidence and complete milestone | `data-management-improvement-v1` at current base `5144e9d10ebf59c2c31ce8a8e3d4fc0152726dfa` → `data-management-evidence-closure-v1` | Ready; exact head `178a483d328d961ab5ad4ad29271e7bc86a8559f`; 581 tests, 100% statement/branch coverage, Ruff, repository validation, actionlint, and `uv lock --check` are green after parent propagation; hosted Checks are queued and no formal approval is observed. | Obtain terminal hosted Checks and an independent same-head review before advancing the milestone. |
| [27](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/27) | Turn data-management gaps into improvement work | `target-state-replan-v1` at current base `371dfe32153192a3e11fcd41016dd82f0a3d70bd` → `data-management-improvement-v1` | Ready; exact head `5144e9d10ebf59c2c31ce8a8e3d4fc0152726dfa`; local 100% statement-and-branch coverage, Ruff, actionlint, repository validation, and diff validation pass after the PR26 security propagation; hosted Checks are newly queued and no formal approval | Obtain independent same-head review and terminal current-head gates while preserving foreign-system ownership. |
| [26](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/26) | Make target-state replanning executable | `target-state-monitoring-v1` → `target-state-replan-v1` | Ready; exact head `371dfe32153192a3e11fcd41016dd82f0a3d70bd`; 533 Python tests with 100% statement-and-branch coverage, Ruff, actionlint, repository validation, and diff validation pass after transaction-isolating the planner fixture and workflow credential repair; hosted Checks are queued; no formal approval | Obtain an independent same-head review and terminal Checks, then verify `gap_detected` semantics and exact replay behavior. |
...
| 33 | `723b310281442b741713648dbe26446631b0a989` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Portfolio summary contract count remains 14 operations; the inferred assessment assertion is scoped to its dimension, the current PR32 parent is propagated, and 705 local Python tests pass with 100% coverage; hosted Checks are queued; the PR is Ready with no formal approval. |
| 32 | `59a560e72863e9609e86f13e7b1d9617cc5fa934` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Current portfolio API head carries the synchronized PR31 parent, dimension-scoped portfolio acceptance, 684 tests with 100% statement-and-branch coverage, Ruff, repository validation, and lock validation; hosted Checks are queued; the PR is Ready with no formal approval. |
| 31 | `2a81aef48c9fc333bc871672ab9f0cc6cf848db5` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Repaired duplicate projection-receipt fixture identity and status-validator schema gate; 651 tests with 100% statement-and-branch coverage, Ruff, repository validation, and lock validation pass; hosted Checks are queued; the PR is Ready with no formal approval, merge, release, or deployment claim. |
| 30 | `a1da69682b7dc54b9410b2310a7fb8ebaceb34a7` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Current ancestor with 616-test/100%-coverage, repository, actionlint, and `uv lock --check` after parent propagation; prior clean PostgreSQL evidence remains historical; hosted Checks are queued; the PR is Ready with no formal approval or protected integration. |
| 29 | `178a483d328d961ab5ad4ad29271e7bc86a8559f` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Current data-management evidence-closure ancestor with the Python support-boundary, uv cooldown, lock metadata, and workflow credential repairs; 581 tests, 100% statement/branch coverage, Ruff, repository validation, actionlint, and `uv lock --check` are green; hosted Checks are queued, the PR is Ready, and no formal approval or protected integration exists. |
| 27 | `5144e9d10ebf59c2c31ce8a8e3d4fc0152726dfa` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Current improvement ancestor includes the PR26 workflow credential hardening; local 100% statement-and-branch coverage, Ruff, actionlint, repository validation, and diff validation pass, while newly pushed hosted Checks are queued and exact-head review is pending; no protected integration or formal approval. |
| 26 | `371dfe32153192a3e11fcd41016dd82f0a3d70bd` | `2026-08-21T07:08:01Z` | `implementation_candidate` | Current target-state replan head; transaction-isolated planner fixture plus workflow credential hardening pass 533 Python tests with 100% statement-and-branch coverage, Ruff, actionlint, repository validation, and diff validation; hosted Checks are queued; Ready; no protected integration or formal approval. |
...
| P1 | Portfolio fit/scoring is normalized but not a buyer API | PR [32](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/32) at exact head `59a560e72863e9609e86f13e7b1d9617cc5fa934` adds the authenticated single-object assessment read port; synchronized PR [33](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/33) at exact head `723b310281442b741713648dbe26446631b0a989` adds the deterministic summary candidate | A portfolio owner still needs an aggregate fit, criticality, cost, and risk answer on the protected branch. | Complete the current-head review and Checks loop for #32, then repeat it for the synchronized summary while preserving tenant, valid-time, recorded-time, truth status, evidence, and next-action fields. | The integrated summary has an OpenAPI contract, purpose-bound reader, deterministic PostgreSQL acceptance, negative authorization tests, and a current-head runtime response. |
| P1 | Append-only database write hot spots lack buyer-observed capacity evidence | PR [35](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/35) at exact head `70594b83253b6c077bb1355c17ece298c22746e5` adds migration 0050, ADR 0025, deterministic 16-bucket tenant routing, fillfactor headroom, hot-write indexes, and prior clean/upgrade PostgreSQL acceptance; PR [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) at exact head `d6e425182e4e5cca34336d3c8bcb4dcded6f9c71` adds a repeatable tenant-scoped capacity snapshot, failure-safe context restoration, and operator runbook; physical partitions and production skew measurements remain absent, and fresh current-head database/runtime evidence is pending | A buyer cannot yet distinguish a measured high-write operating envelope from a schema prepared for future scale. | Complete the exact-head review/Checks loop for PR36, then compare authorized representative-data snapshots for tenant skew, queue lag, write amplification, and index behavior before proposing a physical HASH/LIST cutover. | Protected integration and a repeatable PostgreSQL capacity report bind the same migration, measurements, RLS behavior, and rollback evidence; no production capacity claim is made before then. |
