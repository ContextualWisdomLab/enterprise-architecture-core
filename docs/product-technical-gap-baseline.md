...
**Live refresh:** `2026-08-20T23:27:00Z` — PR #18 was inspected at exact head
`79c8893a7ee16d1577dc998254e7ace2f598580a`, based on protected
...
`a5fb5b11f05eb81a541ad40c5525975fe860c8ff` with 19 hosted Checks queued after
a current-head Ruff repair. Local PR34 evidence passes 126 tests, 100%
...
`192e72f69fa0e212d235d7f010196525d9c8ce1b` with the repeatable tenant-scoped
hot-write snapshot baseline; its clean PostgreSQL 18.6 run passed all 63 SQL
acceptance files, including the new snapshot acceptance. The public Checks
pages currently show PR18 with 10 queued jobs and PR33, PR34, PR35, and PR36
with 19 queued jobs each, with no current-head failures observed. PR14 still
...
latter is addressed by PR34; neither failure is converted to success or rerun
unchanged.
...
| Hot-write capacity preparation | Migration 0050 and ADR 0025 on PR #35 exact head `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` define deterministic tenant-derived 16-bucket routing, storage headroom, and hot-write indexes; PR #36 exact head `192e72f69fa0e212d235d7f010196525d9c8ce1b` adds a read-only tenant-scoped snapshot for row volume, queue lag, bucket, relation/index size, and cumulative tuple/WAL comparison; clean PostgreSQL 18 SQL acceptance passes; physical partition deployment is not claimed | A buyer gets a repeatable measurement baseline, while production capacity and partition cutover still require authorized representative-data evidence. |
...
| ---: | --- | --- | --- | --- |
| [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) | Add repeatable hot-write capacity snapshot | `hot-write-capacity` at current base `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` → `codex/hot-write-capacity-report` | Draft; exact head `192e72f69fa0e212d235d7f010196525d9c8ce1b`; tenant-scoped read-only report and smoke acceptance pass in clean PostgreSQL 18.6 with all 63 SQL acceptance files; hosted Checks queued; no formal approval | Obtain an independent same-head review and terminal Checks; merge only after PR35's protected gate and the exact current stack are valid. |
...
| ---: | --- | --- | --- | --- |
| 36 | `192e72f69fa0e212d235d7f010196525d9c8ce1b` | `2026-08-20T23:27:00Z` | `implementation_candidate` | Tenant-scoped read-only hot-write snapshot and smoke acceptance; clean PostgreSQL 18.6 migration run passed all 63 SQL acceptance files; hosted Checks queued; no formal approval, merge, release, or production-capacity claim. |
...
| 18 | `79c8893a7ee16d1577dc998254e7ace2f598580a` | `2026-08-20T23:27:00Z` | `implementation_candidate` | Documentation snapshot source head; exact-head OpenCode dispatch accepted and its review job is queued; hosted Checks queued; no formal current-head approval observed. |
