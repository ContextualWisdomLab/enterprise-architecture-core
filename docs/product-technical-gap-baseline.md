...
**Snapshot:** 2026-08-20T14:22:40Z, GitHub API and repository inspection<br>
...
**Live refresh:** `2026-08-20T14:22:40Z` — PR #18 is currently at exact head
`da4ca08abe08ecd3fad09829e7bd4355e24c86b3`; its exact-head OpenCode dispatch
was accepted by the central workflow, while hosted required checks remain
queued and no formal review is observed. All previously draft stacked PRs are
ready for review. PR #35 is the hot-write-capacity candidate at exact head
`839e0e025d72222d92bfd844405dd53b7676f67e`, based on PR #33 exact head
`07e59820e927e53a997604e1dd017df07f000121`; its required checks are queued and
its exact-head OpenCode dispatch was accepted. PR #14 is ready at exact head
`ca6889497728e1a3f09d68790a9096576e13a3ff`; its latest required Checks result
passes the runtime and supply-chain checks. Two older failed runs are historical
evidence only and do not transfer to a new head.
...
| ---: | --- | --- | --- | --- |
| [35](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/35) | Prepare append-only write boundaries for hot partitioning | `portfolio-assessment-summary` at current base `07e59820e927e53a997604e1dd017df07f000121` → `hot-write-capacity` | Ready; exact head `839e0e025d72222d92bfd844405dd53b7676f67e`; PR31 fixture repair is included through the synchronized parent chain; hosted required Checks are queued; exact-head OpenCode dispatch was accepted | Wait for terminal Checks and an independent same-head review; merge only after PR #33 and its ancestors are current and protected. |
...
| [33](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/33) | Publish deterministic portfolio assessment summary | `portfolio-assessment-api` at current base `982b02c763db15c0f27d01786004c13c0ee4f7a4` → `portfolio-assessment-summary` | Ready; exact head `07e59820e927e53a997604e1dd017df07f000121`; PR31 fixture repair is included; hosted Checks are queued and exact-head OpenCode dispatch was accepted | Wait for terminal Checks and obtain an independent same-head review; merge only after the synchronized parent and descendant gates are current. |
| [32](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/32) | Expose portfolio assessment read port | `data-management-recheck-status-v1` at current base `d24dddd073d96ad4de6db29750f7a55402d9d08c` → `portfolio-assessment-api` | Ready; exact head `982b02c763db15c0f27d01786004c13c0ee4f7a4`; PR31 fixture repair is included; required Checks are queued and exact-head OpenCode dispatch was accepted | Wait for terminal Checks and obtain an independent same-head review; repair and revalidate before merge. |
| [31](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/31) | Expose data-management reassessment status | `data-management-recheck-v1` at current base `c5b1b78ba3690104f7de32ce76b20bae77c3227b` → `data-management-recheck-status-v1` | Ready; exact head `d24dddd073d96ad4de6db29750f7a55402d9d08c`; PostgreSQL 18 full acceptance now passes; hosted Checks are queued and exact-head OpenCode dispatch was accepted | Wait for terminal Checks, inspect the independent review, then merge only with current-head evidence. |
| [30](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/30) | Request data-management assessment recheck | `data-management-evidence-closure-v1` → `data-management-recheck-v1` | Ready; newest workflow runs queued; exact-head OpenCode review dispatched | Validate the causal evidence boundary and terminal Checks before advancing the stack. |
...
| [18](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/18) | Buyer README and architecture decision records | `develop` → `cursor/ea-core-customer-docs-609e` | Ready; exact head `da4ca08abe08ecd3fad09829e7bd4355e24c86b3`; exact-head OpenCode dispatch was accepted; hosted required-workflow and security/provenance evidence remains queued; no formal review | Obtain an independent same-head review, diagnose only current-head failures, and re-fetch all terminal Checks before merge. |
...
the GitHub REST API and local acceptance runs; the current stacked rows were
refreshed at `2026-08-20T14:22:40Z`. Every open PR is classified as
...
| ---: | --- | --- | --- | --- |
| 35 | `839e0e025d72222d92bfd844405dd53b7676f67e` | `2026-08-20T14:22:40Z` | `implementation_candidate` | Clean PostgreSQL 18 migration plus every SQL acceptance file passed after the PR31 fixture repair; Python 682-test/100% statement-and-branch coverage, Ruff, and repository validation passed; hosted Checks queued; exact-head OpenCode dispatch accepted. |
...
| 33 | `07e59820e927e53a997604e1dd017df07f000121` | `2026-08-20T14:22:40Z` | `implementation_candidate` | PR31 fixture repair is included through the synchronized parent chain; hosted Checks are queued and exact-head OpenCode dispatch was accepted. |
| 32 | `982b02c763db15c0f27d01786004c13c0ee4f7a4` | `2026-08-20T14:22:40Z` | `implementation_candidate` | PR31 fixture repair is included; hosted Checks are queued and exact-head OpenCode dispatch was accepted. The aggregate local chain passed Python and PostgreSQL acceptance. |
| 31 | `d24dddd073d96ad4de6db29750f7a55402d9d08c` | `2026-08-20T14:22:40Z` | `implementation_candidate` | PostgreSQL 18 full migration and SQL acceptance passed with the composite-row fixture correction; hosted Checks are queued and exact-head OpenCode dispatch was accepted. |
| 30 | `7e3e6805c80c37129306538c4d0d16df59341ee5` | `2026-08-20T13:06:28Z` | `implementation_candidate` | Open draft candidate; no protected integration, immutable release, or live deployment evidence observed. |
...
| 18 | `da4ca08abe08ecd3fad09829e7bd4355e24c86b3` | `2026-08-20T14:22:40Z` | `implementation_candidate` | Documentation snapshot refresh is pushed; hosted required workflows/security/provenance Checks queued; exact-head OpenCode dispatch was accepted; no formal current-head review observed. |
