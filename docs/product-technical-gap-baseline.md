...
**Snapshot:** 2026-08-21T02:09:43Z, GitHub inspection and repository inspection<br>
...
**Live refresh:** `2026-08-21T02:09:43Z` — PR #31 was inspected at exact head
...
`fbfef9b361112d8bf472676afcea0f391039a03d`, based on protected
...
`648acc360778f9dfae579745a567e4ec1703e267`, with the same clean PostgreSQL
18 portfolio fixture assertion repair; its hosted Checks are queued and it
remains Draft. No formal same-head approval or protected merge is observed.

PR #36 extends PR35 at exact head
`56386952ad6fe9b64e490209c5e46c320c39e47e` with the operator runbook and
failure-safe tenant-context restoration for the repeatable tenant-scoped
hot-write snapshot; the preceding code head passed targeted PostgreSQL 18.6
context-restoration acceptance, and the current docs-only shell continuation
fix passes diff validation. PR #12 now has exact head
...
PostgreSQL 18.6 installation. The public Checks pages currently show PR12 and
PR36 with 19 queued and 7 skipped jobs each, and PR18 with queued jobs; all
...
| Hot-write capacity preparation | Migration 0050 and ADR 0025 on PR #35 exact head `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` define deterministic tenant-derived 16-bucket routing, storage headroom, and hot-write indexes; PR #36 exact head `56386952ad6fe9b64e490209c5e46c320c39e47e` adds a read-only tenant-scoped snapshot, failure-safe context restoration, and operator runbook for row volume, queue lag, bucket, relation/index size, and cumulative tuple/WAL comparison; targeted PostgreSQL 18.6 acceptance passed on the preceding code head and the current docs-only correction passes diff validation; physical partition deployment is not claimed | A buyer gets a repeatable measurement baseline, while production capacity and partition cutover still require authorized representative-data evidence. |
...
| ---: | --- | --- | --- | --- |
| [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) | Add repeatable hot-write capacity snapshot | `hot-write-capacity` at current base `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` → `codex/hot-write-capacity-report` | Draft; exact head `56386952ad6fe9b64e490209c5e46c320c39e47e`; tenant-scoped read-only report retains caller-context restoration, and the current docs-only shell continuation fix passes diff validation; hosted Checks 19 queued and 7 skipped; no formal approval | Obtain an independent same-head review and terminal Checks; merge only after PR35's protected gate and the exact current stack are valid. |
...
| [18](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/18) | Buyer README and architecture decision records | `develop` at current base `1c0fa8b15ceb9e72186274aeb255d6777eb84ef4` → `cursor/ea-core-customer-docs-609e` | Draft; documentation refresh based on source parent `fbfef9b361112d8bf472676afcea0f391039a03d`; evidence baseline includes the current PR12, PR16, scheduler, PR34, PR35, PR36, and target-state fixture repairs; hosted Checks are queued; no formal approval | Keep the external Draft state intact, then obtain an independent same-head review and re-fetch all terminal Checks before merge. |
...
| ---: | --- | --- | --- | --- |
| 36 | `56386952ad6fe9b64e490209c5e46c320c39e47e` | `2026-08-21T02:09:43Z` | `implementation_candidate` | Tenant-scoped read-only hot-write snapshot plus operator runbook; the preceding code head passed targeted PostgreSQL 18.6 context-restoration acceptance, and the current docs-only shell continuation fix passes diff validation; hosted Checks 19 queued and 7 skipped; no formal approval, merge, release, or production-capacity claim. |
...
| 18 | `fbfef9b361112d8bf472676afcea0f391039a03d` | `2026-08-21T02:09:43Z` | `implementation_candidate` | Documentation snapshot source parent; evidence baseline includes the current PR12 projection repair, PR16, scheduler, PR34, PR35, PR36 documentation correction, and target-state fixture repairs; hosted Checks queued; Draft; no formal current-head approval observed. |
