...
**Live refresh:** `2026-08-21T00:34:02Z` — PR #16 was inspected at exact head
...
`55bf0a94f94f2dfa66dcd1151cb701c9ea8bc813`, based on protected
...
`1be0c20cf7111869c6ef62a926456d98fd76d21a` with the operator runbook for the
...
| Hot-write capacity preparation | Migration 0050 and ADR 0025 on PR #35 exact head `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` define deterministic tenant-derived 16-bucket routing, storage headroom, and hot-write indexes; PR #36 exact head `1be0c20cf7111869c6ef62a926456d98fd76d21a` adds a read-only tenant-scoped snapshot and operator runbook for row volume, queue lag, bucket, relation/index size, and cumulative tuple/WAL comparison; clean PostgreSQL 18 SQL acceptance passes; physical partition deployment is not claimed | A buyer gets a repeatable measurement baseline, while production capacity and partition cutover still require authorized representative-data evidence. |
...
| ---: | --- | --- | --- | --- |
| [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) | Add repeatable hot-write capacity snapshot | `hot-write-capacity` at current base `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` → `codex/hot-write-capacity-report` | Draft; exact head `1be0c20cf7111869c6ef62a926456d98fd76d21a`; tenant-scoped read-only report, operator runbook, and smoke acceptance pass in clean PostgreSQL 18.6 with all 63 SQL acceptance files; hosted Checks queued; no formal approval | Obtain an independent same-head review and terminal Checks; merge only after PR35's protected gate and the exact current stack are valid. |
...
| ---: | --- | --- | --- | --- |
| 36 | `1be0c20cf7111869c6ef62a926456d98fd76d21a` | `2026-08-20T23:44:50Z` | `implementation_candidate` | Tenant-scoped read-only hot-write snapshot plus operator runbook; clean PostgreSQL 18.6 migration run on the preceding SQL head passed all 63 SQL acceptance files; hosted Checks queued; no formal approval, merge, release, or production-capacity claim. |
...
| 18 | `55bf0a94f94f2dfa66dcd1151cb701c9ea8bc813` | `2026-08-21T00:34:02Z` | `implementation_candidate` | Documentation snapshot source head; evidence baseline includes the current PR16 and scheduler updates; hosted Checks queued; no formal current-head approval observed. |
