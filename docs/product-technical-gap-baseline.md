...
**Live refresh:** `2026-08-21T01:27:22Z` — PR #16 was inspected at exact head
...
`f8196a0653a7d22d806d9cd2f62c19b36bb99021`, based on protected
...
| ---: | --- | --- | --- | --- |
| [36](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/36) | Add repeatable hot-write capacity snapshot | `hot-write-capacity` at current base `6eab7bae3e2e7424ac33fbd74a3953e2a3949596` → `codex/hot-write-capacity-report` | Ready; exact head `545245ef9f780bcdc321fc2223efa609429479a2`; tenant-scoped read-only report now restores caller context on success and failure, targeted PostgreSQL 18.6 acceptance passes, and the preceding head passed all 63 SQL acceptance files; hosted Checks queued; no formal approval | Obtain an independent same-head review and terminal Checks; merge only after PR35's protected gate and the exact current stack are valid. |
...
| [18](https://github.com/ContextualWisdomLab/enterprise-architecture-core/pull/18) | Buyer README and architecture decision records | `develop` at current base `1c0fa8b15ceb9e72186274aeb255d6777eb84ef4` → `cursor/ea-core-customer-docs-609e` | Draft; current head `860baf0371534c0ac8229f9055e37685c468ad6c` is the documentation refresh based on source parent `f8196a0653a7d22d806d9cd2f62c19b36bb99021`; evidence baseline includes the current PR16, scheduler, PR34, PR35, PR36, and target-state fixture repairs; hosted Checks are queued; no formal approval | Keep the external Draft state intact, then obtain an independent same-head review and re-fetch all terminal Checks before merge. |
...
| 18 | `860baf0371534c0ac8229f9055e37685c468ad6c` | `2026-08-21T01:29:00Z` | `implementation_candidate` | Current documentation head; refresh is based on source parent `f8196a0653a7d22d806d9cd2f62c19b36bb99021`; evidence baseline includes the current PR16, scheduler, PR34, PR35, PR36, and target-state fixture repairs; hosted Checks queued; Draft; no formal current-head approval observed. |
