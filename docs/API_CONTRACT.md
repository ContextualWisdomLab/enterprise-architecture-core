...
- Reassessment status is read-only. It follows append-only supersession to current evidence; only `authoritative` or `observed` current successor truth may drive ordinary evidence-gap/evidence-complete actions, while weaker truth origins require explicit review.
