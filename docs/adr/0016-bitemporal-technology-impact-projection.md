...
The requested technology version, valid-time cutoff, system-recording cutoff, and planning horizon are mandatory. `NULL` for any of those inputs fails closed with a check violation instead of being interpreted as an unbounded or unknown query. The planning horizon is additionally bounded to 1..3650 days.
...
An active `end_of_life`/`retired` lifecycle state escalates to remediation once the EA-owned path is complete and not awaiting truth review. Active phase-out or the earliest recorded `phase_out`/`end_of_life`/`retired` transition inside the explicit planning horizon requests target-state planning only when the risk-driving lifecycle evidence is present. If the current lifecycle interval or a within-horizon risk-driving future transition lacks an evidence identifier, the row remains risk-visible but changes to `missing_lifecycle_evidence` / `complete_lifecycle_evidence`; the projector does not emit a decision-ready remediation or target-state action from provenance-incomplete lifecycle state.
...
- Missing temporal cutoffs/horizons are rejected rather than silently collapsing the result set or widening the query.
...
`database/tests/zz_verify_technology_change_impact_guards.sql` adds fail-closed regressions for missing valid-time/system-time cutoffs and planning horizon. It also removes the evidence identifier from a within-horizon risk-driving lifecycle transition inside a transaction and proves the projector preserves the risk status while withholding `plan_target_state` in favor of `complete_lifecycle_evidence`, then rolls the fixture mutation back.
