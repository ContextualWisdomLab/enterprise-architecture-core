...
5. The dependency-aware command first owns the source-assessment serialization point, then locks every existing requested prerequisite in deterministic UUID order before evaluating its active/superseded state. This makes prerequisite acceptance linearizable with concurrent supersession while avoiding dependency-set lock-order cycles. It then delegates creation of the proposed plan, milestone, and transactional-outbox evidence to the existing purpose-bound command and inserts the normalized dependency set in the same transaction. Any dependency failure rolls back the entire decision.
...
Concurrent prerequisite supersession cannot pass a stale liveness check merely because an improvement decision was waiting on the source assessment row. Different source assessments that share prerequisite initiatives acquire those prerequisite locks in deterministic UUID order.
...
- Dependency/replay PostgreSQL buyer acceptance: `database/tests/zzzzzzzzzzzzzzzzzzzzz_verify_data_management_dependencies.sql`.
- Concurrent supersession RED/GREEN acceptance: `database/tests/zzzzzzzzzzzzzzzzzzzzzz_verify_data_management_dependency_serialization.sql`.
- Schema implementation, prerequisite serialization, and replay guard: `database/migrations/0036_improvement_plan_pair_integrity.sql`.
