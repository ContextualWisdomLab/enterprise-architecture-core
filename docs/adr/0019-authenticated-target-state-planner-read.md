...
A missing or partial Keyverse configuration, unsafe JWKS location, network/key/signature failure, invalid issuer/audience/time/tenant/role claim, malformed bitemporal query, or database/query-port failure is non-passing. The service returns a stable error code and actionable next step without exposing token, DSN, SQL, or credential material.
