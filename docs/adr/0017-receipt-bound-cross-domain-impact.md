...
Migration 0019 makes receipt evidence append-preserving. Tenant/id/source/event-id/digest/schema/received-time identity fields are immutable after insertion; hard deletion is rejected; `processed` and `rejected` receipts are terminal and cannot later be rewritten. A failed receipt may still re-enter processing, while `processing -> received` regression is rejected. This preserves retryability without allowing already accepted evidence to disappear or change underneath an earlier system-time decision.
...
- Processed/rejected receipt evidence cannot be hard-deleted or retroactively reclassified; failed receipt processing can still be retried.
...
Repository validation binds the resulting schema inventory to 38 tables, 381 validator-visible typed column declarations, 12 indexes, and 320 named constraints. The PostgreSQL CI lane must prove clean install, dynamic upgrade through migration 0019, migration-ledger integrity, all SQL invariants, and runtime RLS acceptance before this branch may be considered green.
