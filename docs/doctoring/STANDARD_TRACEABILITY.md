...
| Bitemporal facts | Architecture audit requirement plus PostgreSQL temporal/range semantics | revision, relation, lifecycle, identity, assessment, objective, initiative, link, and milestone valid/system intervals; ADRs 0004, 0013, 0014 |
...
| Append-preserving strategy decisions | Architecture decision auditability requirement | ADR 0014; semantic-update rejection, one-time supersession tests, planned append-only scenario/transformation continuation |
...
| Tenant data isolation | PostgreSQL 18 row-level security | forced policies and non-superuser runtime/assessment/strategy acceptance; defense-in-depth, not caller authentication |
